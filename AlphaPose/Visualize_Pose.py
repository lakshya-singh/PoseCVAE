...
path= './sliding_window_output/01/14_0001_1_1.npy'
...
image_path = '/home/SharedData/lakshya/frames/train/01/0001.jpg'
...
# Save
cv2.imwrite("./visualize_pose_output/result.png",image)
...
path= './sliding_window_output/01/'
files = sorted(os.listdir(path))
for file in files:
    os.makedirs('./visualize_pose_output_2/'+file)
    data= np.load(path+file)
    for im_res in data:
        im_name = im_res[2]
        img= vis_frame(im_res)
        new_path = os.path.join('./visualize_pose_output_2/', file, str(im_name)+'.jpg')
        print(new_path)
...
files = os.listdir('./sliding_window_output/01/')
path = './sliding_window_output/01/'
y = list()
x = list()
for file in files:
