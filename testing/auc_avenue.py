...
def read_numpy_concat_mean(path):
	...
	for ele in k:
		...
		current_file=np.load(os.path.join(path,ele))
		#np.mean(curren_file)
		#np.std(curren_file)
...
# path=path_directory+'/final_mse/'+str(arg_dir)+'/'
path=os.path.join(path_directory,'final_mse')
gt_path=os.path.join(path_directory,'ground_truth')
...
print('{}: auc_score : {}'.format(current_time, auc_value),file=log)
print('{}: auc_score : {}'.format(current_time, auc_value))
