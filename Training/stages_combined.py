...
log = open(os.path.join(path_directory,"log/var_conv_ood_7.txt"), "a")
...
train_folder = 'sliding_window_train'
val_folder = 'sliding_window_val'

################ Train Data Dictionary #########################
train_files=os.listdir(os.path.join(path_directory,train_folder))
dict_train={}
index=0
for file_3 in train_files:
    i=os.path.join(path_directory,train_folder,file_3)
    dict_train[index]=i
    index=index+1
...
val_files=os.listdir(os.path.join(path_directory,val_folder))
dict_val={}
index=0
for file_3 in val_files:
	i=os.path.join(path_directory,val_folder,file_3)
	dict_val[index]=i
	index=index+1
	#print(i)
...
class dataset_train(Dataset):
	...
	def __getitem__(self, idx):
		...
		self.data=np.load(dict_train[idx],allow_pickle=True)
...
class dataset_val(Dataset):
	...
	def __getitem__(self, idx):
		...
		self.data=np.load(dict_val[idx],allow_pickle=True)
...
batch_size = 128
# n_epochs = 2000
# n_epochs = 160+250+400+2
# n_epochs = 500+400+100+2
...
# '''
# checkpoint = torch.load('./checkpoints/stage2/train_400.pth') #checkpoint_class0_400.pth
# print(checkpoint)
...
############### with simulated abnormal data ##########
        ...
        # WHY THERE IS NO train_loss += loss.item() ?????
...
for e in range(epoch,n_epochs):
        if(e<=pretr):
            ...
            print(f'Pretrain:: Epoch {e}, Train Loss:  {train_loss:.3f}, Val_loss: {val_loss:.3f}, Learning_rate: {get_lr(optimizer)}',file=log)
            if(e%20==0):
	            torch.save({
            'epoch': e,
            ...
            'val_loss': val_loss
            }, './checkpoints/stage1/pretrain_'+str(e)+'.pth')

            if(e == pretr):
                files = os.listdir('./checkpoints/stage1/')
                files = sorted(files,key=numericalSort)
                last_file = files[-1]

                checkpoint = torch.load(f'./checkpoints/stage1/{last_file}')
                # print(checkpoint)
                model.load_state_dict(checkpoint['model_state_dict'])
                optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
                # epoch = checkpoint['epoch'] + 1
                print("Loading Epoch:",e)
                train_loss = checkpoint['train_loss']
                val_loss = checkpoint['val_loss']
                print(f'train_loss :: {train_loss}, val_loss :: {val_loss}')

        elif(e>pretr and e<=tr):
            # pass
            ...
            if(e%20==0):
                ...
                scheduler.step(val_loss)
                print(f'Epoch {e}, Learning_rate: {get_lr(optimizer)}')
            print(f'Train:: Epoch {e}, Train Loss:  {train_loss:.3f}, Val_loss: {val_loss:.3f}')
            print(f'Train:: Epoch {e}, Train Loss:  {train_loss:.3f}, Val_loss: {val_loss:.3f}, Learning_rate: {get_lr(optimizer)}',file=log)
            if(e%20==0):
                torch.save({
            'epoch': e,
            'model_state_dict': model.state_dict(),
            ...
            'train_loss': train_loss,
            'val_loss': val_loss
            }, './checkpoints/stage2/train_'+str(e)+'.pth')

            if(e == tr):
                files = os.listdir('./checkpoints/stage2/')
                files = sorted(files,key=numericalSort)
                last_file = files[-1]

                checkpoint = torch.load(f'./checkpoints/stage2/{last_file}')
                # print(checkpoint)
                model.load_state_dict(checkpoint['model_state_dict'])
                optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
                # epoch = checkpoint['epoch'] + 1
            ...
            if (e % 20 == 0):
                ...
                scheduler.step(val_loss)
                print(f'Epoch {e}, Learning_rate: {get_lr(optimizer)}')
            print(f'BCETrain:: Epoch {e}, Train Loss:  {train_loss:.3f}, Val_loss: {val_loss:.3f}')
            print(f'BCETrain:: Epoch {e}, Train Loss:  {train_loss:.3f}, Val_loss: {val_loss:.3f}, Learning_rate: {get_lr(optimizer)}',file=log)
            if(e%20==0):
                torch.save({
            'epoch': e,
            'model_state_dict': model.state_dict(),
            ...
            'train_loss': train_loss,
            'val_loss': val_loss
            }, './checkpoints/stage3/bcetrain_'+str(e)+'.pth')
