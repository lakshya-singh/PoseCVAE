...
checkpoint_input=str(sys.argv[1])

sliding_win_len = 7

path_directory = './' # base working directory
test_data = "sliding_window" # folder in which test tracks are stored
...
input_dim = 2*4*num_channels*1
hidden_dim1 = 2*4*num_channels*1
hidden_dim2 = 2*2*num_channels*1
latent_dim = 2*num_channels*1
cond_dim = 2*4*num_channels*1

# test_folder=os.listdir(path_directory+'/test/'+ test_data +'/')
direct = os.path.join(path_directory,test_data)
test_folder=os.listdir(direct)

list_test=[]
for video_no in test_folder:
	track_path =os.path.join(path_directory,test_data,video_no)
	video=os.listdir(track_path) #comprises of a list all the tracks corresponding to a video
	
	for track in video:
		if(track!='result'):
			i=os.path.join(track_path,track)
			list_test.append(i)
...
#data_conf has the confidence values
		...
		return self.data_cord_gt,self.data_conf_gt, self.data_cord_condition,self.data_conf_condition, self.frames, self.name_file, self.track_path
####################################################################################
...
# model.load_state_dict(torch.load(path_directory+'/checkpoints/vae_epoch_'+checkpoint_input+'.pth'))

#CHK checkpoint_abnormal-bce_600
load_path = os.path.join(path_directory,'checkpoint',checkpoint_input)
...
def test():
	model.eval()
	test_loss = 0
	print(" LOADED checkpoint ",checkpoint_input)
	...
	with torch.no_grad():
		for i,data in enumerate(testloader):
			...
			for i in range(0,x.shape[0]):
						#name_a=14_videoNUmber
		                # name_a='_'.join([name[i].split('_')[0],name[i].split('_')[1]])
		                name_a = str('%02d' % int(name[i].split('_')[1]))
		                
		                # print("name_a = ",name_a)
		                # print("name[i] = ",name[i])
		                
		                g=os.listdir(os.path.join(path_directory,'frames',name_a))
		                # print(name[i],end=' ')
		                # print(name_a)
		                ...
		                # print("\n npy used's shape = ",npy[0,start[sliding_win_len - 1]:start[-1]].shape,"\n")
		
		                npy[0,start[sliding_win_len - 1]:start[-1]] = error[i]
		                #result1 = result[i,:,:]

		                pth = os.path.join(path_directory,test_data,track_path[i],'result',name[i])
