...
s=os.path.join(path_directory,test_data_folder)
...
for elemet_vid in video_lst:
	print(elemet_vid)
	k=os.listdir(os.path.join(s,elemet_vid,'result')) #.npy file for each person, NOW A MAX WILL BE TAKEN after concat.
	#print('after')	

	e=os.listdir(os.path.join(path_directory,'frames',elemet_vid)) #original TEST FRAMES
	...
	for ele in k:
		count = count + 1
		#print('path',count)
		g=np.load(os.path.join(s,elemet_vid,'result',ele))
	...
	np.save(path_directory+'/final_mse'+'/'+elemet_vid+'.npy',res)
...
############### gt is the ground truth numpy file #################
	...
	print(os.path.join(path_directory,'mse_plots',elemet_vid+'.png'))

	plt.legend(loc = 'lower right')
	plt.savefig(os.path.join(path_directory,'mse_plots',elemet_vid+'.png'))
	plt.close()
