...
def main(format, filename, hrnet_m, hrnet_c, hrnet_j, hrnet_weights, image_resolution, single_person, use_tiny_yolo,
         max_batch_size, csv_output_filename, csv_delimiter, json_output_filename, device, outdir):
    ...
    while True:
        ...
        if format == 'csv':
            ...
            for j, pt in enumerate(pts):
                json_data = {}
                json_data['keypoints'] = list()
                json_data['image_id'] = str(index+1)+'.jpg'
                json_data['category_id'] = 1
                # temp.append(pt.tolist())
                temp = np.array(pt).flatten()
                json_data['keypoints'] = temp.tolist()
                json_data['score'] = 2.1003262996673584
...
if __name__ == '__main__':
    ...
    ''' In --outdir default directory is the current directory, which will result in the over-writing of output.json created
    if this file is called multiple times '''
    parser.add_argument("--outdir", help="pose json parent-folder", type=str, default='./')
